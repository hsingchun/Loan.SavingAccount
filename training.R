...
t4.1
    ...
    #t4.1 test
    trainSplit$TA_LOAN<-as.factor(trainSplit$TA_LOAN)
    pred4.1.v.p <-predict(t4.1, trainSplit, type = "prob")
    head(pred4.1.v.p,3)
    #---Result matrix----
    res.tree<-matrix(data = NA,nrow = nrow(trainSplit),ncol = 5)
    colnames(res.tree)=c("ID","TA_LOAN","PredictProb","Cutoff","Class")
    #------SET CUTOFF----
    Cutoff<-0.4
    prediction<-as.numeric(pred4.1.v.p[,2]>Cutoff)
    res.tree[,1]<-trainSplit[,2]
    res.tree[,2]<-as.numeric(as.character(trainSplit[,1]))
    ...
    #prediction<-as.numeric(pred4.1.v.p[,2]>Cutoff)
    #confusionMatrix(prediction,trainSplit$TA_LOAN,positive = "1")
    
    write.table(res.tree,file = "tr0+C50.csv",sep = ",")
...
# ====== 0+RF ======
    ...
    rf.test
    # rf.test test   
    Pred.rf1.v <- predict(rf.test, trainSplit,type = "prob")
    head(Pred.rf1.v,10)
    
    #---Result matrix----
    res.rf<-matrix(data = NA,nrow = nrow(trainSplit),ncol = 5)
    colnames(res.rf)=c("ID","TA_LOAN","PredictProb","Cutoff","Class")
    #------SET CUTOFF----
    Cutoff<-0.3
    prediction<-as.numeric(Pred.rf1.v[,2]>Cutoff)
    res.rf[,1]<-trainSplit[,2]
    res.rf[,2]<-as.numeric(as.character(trainSplit[,1]))
    ...
    #prediction<-as.numeric(Pred.rf1.v[,2]>Cutoff)
    #confusionMatrix(prediction,trainSplit$TA_LOAN,positive = "1")
    write.table(res.rf,file = "tr0+RF.csv",sep = ",")
...
# ====== 0+XGB ======
    ...
    # predict values in 
    gg.v=matrix(round(unlist(trainSplit),3),length(trainSplit[,1]),ncol(loan))
    colnames(gg.v)=names(trainSplit)
    head(gg.v,2)
    table(gg.v[,1])
    ...
    head(x.pred.v,2)
    #---Result matrix----
    res.XGB<-matrix(data = NA,nrow = nrow(trainSplit),ncol = 5)
    ...
    write.table(res.XGB,file = "tr0+XGB.csv",sep = ",")
...
# ============== $$ ROSE+ $$==============
    ...
    trainSplit$TA_LOAN<-as.factor(trainSplit$TA_LOAN)
    pred5.1 <-predict(t5.1, trainSplit, type = "prob")
    head(pred5.1,3)
    #---Result matrix----
    res.tree<-matrix(data = NA,nrow = nrow(trainSplit),ncol = 5)
    colnames(res.tree)=c("ID","TA_LOAN","PredictProb","Cutoff","Class")
    ...
    #prediction<-as.numeric(pred5.1[,2]>Cutoff)
    #confusionMatrix(prediction,trainSplit$TA_LOAN,positive = "1")
    write.table(res.tree,file = "trR+C50.csv",sep = ",")
    ...
    #prediction<-as.numeric(Pred.rf1.v[,2]>Cutoff)
    #confusionMatrix(prediction,trainSplit$TA_LOAN,positive = "1")
    write.table(res.rf,file = "trR+RF.csv",sep = ",")
    # ====== R+XGB ======
    ...
    write.table(res.XGB,file = "trR+XGB.csv",sep = ",")
...
# ============== $$ SMOTE+ $$==============
    ...
    # tclassTree  test
    predS<-predict(classTree,trainSplit, type = "prob")
    head(predS,3)
    #--Result matrix----
    res.tree<-matrix(data = NA,nrow = nrow(trainSplit),ncol = 5)
    colnames(res.tree)=c("ID","TA_LOAN","PredictProb","Cutoff","Class")
    ...
    write.table(res.tree,file = "trSMO+C50.csv",sep = ",")
    ...
    write.table(res.rf,file = "trSMO+RF.csv",sep = ",")
    ...
    write.table(res.XGB,file = "trSMO+XGB.csv",sep = ",")
