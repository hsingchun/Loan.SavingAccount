...
t4.1
    ...
    #t4.1 test
    testSplit$TA_LOAN<-as.factor(testSplit$TA_LOAN)
    pred4.1.v.p <-predict(t4.1, testSplit, type = "prob")
    head(pred4.1.v.p,3)
    #---Result matrix----
    res.tree<-matrix(data = NA,nrow = nrow(testSplit),ncol = 5)
    colnames(res.tree)=c("ID","TA_LOAN","PredictProb","Cutoff","Class")
    #------SET CUTOFF----
    Cutoff<-0.4
    prediction<-as.numeric(pred4.1.v.p[,2]>Cutoff)
    res.tree[,1]<-testSplit[,2]
    res.tree[,2]<-as.numeric(as.character(testSplit[,1]))
    ...
    #prediction<-as.numeric(pred4.1.v.p[,2]>Cutoff)
    #confusionMatrix(prediction,testSplit$TA_LOAN,positive = "1")
    
    write.table(res.tree,file = "t0+C50.csv",sep = ",")
...
# ====== 0+RF ======
    ...
    rf.test
    # rf.test test   
    Pred.rf1.v <- predict(rf.test, testSplit,type = "prob")
    head(Pred.rf1.v,10)
    
    #---Result matrix----
    res.rf<-matrix(data = NA,nrow = nrow(testSplit),ncol = 5)
    colnames(res.rf)=c("ID","TA_LOAN","PredictProb","Cutoff","Class")
    #------SET CUTOFF----
    Cutoff<-0.3
    prediction<-as.numeric(Pred.rf1.v[,2]>Cutoff)
    res.rf[,1]<-testSplit[,2]
    res.rf[,2]<-as.numeric(as.character(testSplit[,1]))
    ...
    write.table(res.rf,file = "t0+RF.csv",sep = ",")
    
# ====== 0+XGB ======
    table(testSplit$TA_LOAN)
    trainSplit$TA_LOAN<-as.character(trainSplit$TA_LOAN)
    trainSplit$TA_LOAN<-as.numeric(trainSplit$TA_LOAN)
    
    table(testSplit$TA_LOAN)
    testSplit$TA_LOAN<-as.character(testSplit$TA_LOAN)
    testSplit$TA_LOAN<-as.numeric(testSplit$TA_LOAN)
    ...
    gg.v=matrix(round(unlist(testSplit),3),length(testSplit[,1]),ncol(loan))
    ...
    head(x.pred.v,2)
    #---Result matrix----
    res.XGB<-matrix(data = NA,nrow = nrow(testSplit),ncol = 5)
    ...
    write.table(res.XGB,file = "t0+XGB.csv",sep = ",")
...
# ============== $$ ROSE+ $$==============
    ...
    testSplit$TA_LOAN<-as.factor(testSplit$TA_LOAN)
    pred5.1 <-predict(t5.1, testSplit, type = "prob")
    head(pred5.1,3)
    #---Result matrix----
    res.tree<-matrix(data = NA,nrow = nrow(testSplit),ncol = 5)
    colnames(res.tree)=c("ID","TA_LOAN","PredictProb","Cutoff","Class")
    ...
    write.table(res.tree,file = "tR+C50.csv",sep = ",")
    ...
    write.table(res.rf,file = "tR+RF.csv",sep = ",")
    # ====== R+XGB ======
    ...
    write.table(res.XGB,file = "tR+XGB.csv",sep = ",")
...
# ============== $$ SMOTE+ $$==============
    ...
    # tclassTree  test
    predS<-predict(classTree,testSplit, type = "prob")
    head(predS,3)
    #--Result matrix----
    res.tree<-matrix(data = NA,nrow = nrow(testSplit),ncol = 5)
    colnames(res.tree)=c("ID","TA_LOAN","PredictProb","Cutoff","Class")
    ...
    write.table(res.tree,file = "tSMO+C50.csv",sep = ",")
    ...
    write.table(res.rf,file = "tSMO+RF.csv",sep = ",")
    ...
    write.table(res.XGB,file = "tSMO+XGB.csv",sep = ",")
