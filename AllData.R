...
t4.1
    ...
    #t4.1 test
    loan$TA_LOAN<-as.factor(loan$TA_LOAN)
    pred4.1.v.p <-predict(t4.1, loan, type = "prob")
    head(pred4.1.v.p,3)
    #---Result matrix----
    res.tree<-matrix(data = NA,nrow = nrow(loan),ncol = 5)
    colnames(res.tree)=c("ID","TA_LOAN","PredictProb","Cutoff","Class")
    #------SET CUTOFF----
    Cutoff<-0.4
    prediction<-as.numeric(pred4.1.v.p[,2]>Cutoff)
    res.tree[,1]<-loan[,2]
    res.tree[,2]<-as.numeric(as.character(loan[,1]))
    ...
    #prediction<-as.numeric(pred4.1.v.p[,2]>Cutoff)
    #confusionMatrix(prediction,loan$TA_LOAN,positive = "1")
    
    write.table(res.tree,file = "a0+C50.csv",sep = ",")
...
# ====== 0+RF ======
    ...
    rf.test
    # rf.test test   
    Pred.rf1.v <- predict(rf.test, loan,type = "prob")
    head(Pred.rf1.v,10)
    
    #---Result matrix----
    res.rf<-matrix(data = NA,nrow = nrow(loan),ncol = 5)
    colnames(res.rf)=c("ID","TA_LOAN","PredictProb","Cutoff","Class")
    #------SET CUTOFF----
    Cutoff<-0.3
    prediction<-as.numeric(Pred.rf1.v[,2]>Cutoff)
    res.rf[,1]<-loan[,2]
    res.rf[,2]<-as.numeric(as.character(loan[,1]))
    ...
    #prediction<-as.numeric(Pred.rf1.v[,2]>Cutoff)
    #confusionMatrix(prediction,loan$TA_LOAN,positive = "1")
    write.table(res.rf,file = "a0+RF.csv",sep = ",")
...
# ====== 0+XGB ======
    ...
    table(loan$TA_LOAN)
    loan$TA_LOAN<-as.character(loan$TA_LOAN)
    ...
    gg.v=matrix(round(unlist(loan),3),length(loan[,1]),ncol(loan))
    colnames(gg.v)=names(loan)
    head(gg.v,2)
    table(gg.v[,1])
    ...
    head(x.pred.v,2)
    #---Result matrix----
    res.XGB<-matrix(data = NA,nrow = nrow(loan),ncol = 5)
    ...
    write.table(res.XGB,file = "a0+XGB.csv",sep = ",")
...
# ============== $$ ROSE+ $$==============
    ...
    loan$TA_LOAN<-as.factor(loan$TA_LOAN)
    pred5.1 <-predict(t5.1, loan, type = "prob")
    head(pred5.1,3)
    #---Result matrix----
    res.tree<-matrix(data = NA,nrow = nrow(loan),ncol = 5)
    colnames(res.tree)=c("ID","TA_LOAN","PredictProb","Cutoff","Class")
    ...
    #prediction<-as.numeric(pred5.1[,2]>Cutoff)
    #confusionMatrix(prediction,loan$TA_LOAN,positive = "1")
    write.table(res.tree,file = "aR+C50.csv",sep = ",")
    ...
    #prediction<-as.numeric(Pred.rf1.v[,2]>Cutoff)
    #confusionMatrix(prediction,loan$TA_LOAN,positive = "1")
    write.table(res.rf,file = "aR+RF.csv",sep = ",")
    # ====== R+XGB ======
    ...
    write.table(res.XGB,file = "aR+XGB.csv",sep = ",")
...
# ============== $$ SMOTE+ $$==============
    ...
    # tclassTree  test
    predS<-predict(classTree,loan, type = "prob")
    head(predS,3)
    #--Result matrix----
    res.tree<-matrix(data = NA,nrow = nrow(loan),ncol = 5)
    colnames(res.tree)=c("ID","TA_LOAN","PredictProb","Cutoff","Class")
    ...
    write.table(res.tree,file = "aSMO+C50.csv",sep = ",")
    ...
    write.table(res.rf,file = "aSMO+RF.csv",sep = ",")
    ...
    write.table(res.XGB,file = "aSMO+XGB.csv",sep = ",")
